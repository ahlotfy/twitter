...
const Interactions = ({
  ...
  const { displayName, email, uid }: any = userInfo;
  // Ref
  const LikeRef: any = useRef();
  // UseEffect
  useEffect(() => {
    if (uid) {
      const collRef = collection(db, "users", `${uid}`, "likes");
      onSnapshot(collRef, (querySnapShot) => {
        const tweetsId = querySnapShot.docs.map((doc) => doc?.id);
        setLikes(tweetsId);
      });
    }
    const coll2Ref = collection(db, "tweets", `${createdTweet}`, "likes");
  ...
  const likeFun = () => {
    if (auth?.currentUser) {
      ...
      const likesList = doc(db, "tweets", `${createdTweet}`, "likes", `${uid}`);
      ...
      if (getLikes?.includes(createdTweet)) {
        deleteDoc(userLikesList);
      } else {
        setDoc(userLikesList, { likeCreated: time });
      }
    } else {
      handleSignIn();
    }
  ...
  const RetweetFun = () => {
    if (auth?.currentUser) {
      if (createdRetweet) {
        ...
        deleteDoc(deleteRetweetRef);
      } else {
        const retweet = doc(db, "users", `${uid}`, `retweet`, `${time}`);
        setDoc(retweet, {
  ...
  const items: MenuProps["items"] = [
    {
      ...
      label: (
        <Option>
          ...
          <p> {createdRetweet ? "Undo repost" : "Retwet"}</p>
        </Option>
      ),
      onClick: () => RetweetFun(),
    },
  ...
  return (
    <InteractionsContent>
      <Reply
        title="Reply"
        className={`${isShowReply ? "active" : ""} `}
        onClick={() => {
          toTweetId();
          if (router?.asPath === `/tweets/${createdTweet}`) {
      ...
      >
        ...
        <ReplyCount>{ReplyCounts != 0 && ReplyCounts}</ReplyCount>
      </Reply>
      ...
      <Like title="Like" ref={LikeRef} onClick={() => likeFun()}>

...
export const Following = () => {
  ...
  useEffect(() => {
    ...
    const unSubcribe = onAuthStateChanged(auth, (user) => {
      if (user || userId) {
        const collRef =
          location?.pathname === `/profile/${userId}/following`
        ...
        const collRef2 = collection(db, "users", `${user?.uid}`, "following");
        onSnapshot(collRef2, (querySnapShot) => {
          querySnapShot?.docs.map((doc) => {
  ...
  useEffect(() => {
    if (loading === false) {
      ...
      onSnapshot(collRef, (querySnapShot) => {
        ...
        SET_USERS_DATA(data?.filter((u) => userFollowing?.includes(u?.uid)));
